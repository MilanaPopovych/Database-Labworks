...
* **Проблема:** У таблиці `students_2nf` атрибути `faculty_name` та `building` залежать від `group_name` (ФЗ 4, ФЗ 5). `curator_name` зберігається як звичайний текст, що дублює сутність викладача і може призвести до невідповідностей при зміні куратора.
