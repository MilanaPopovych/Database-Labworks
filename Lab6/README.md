...
- `schema.prisma` - основний файл схеми, куди за допомогою команди `npx prisma db pull` було імпортовано нашу поточну структуру таблиць.
...
В результаті виконання лабораторної роботи, було застосовано інструментарій Prisma для автоматизації міграцій бази даних та декларативного моделювання зв'язків.
Використання `prisma migrate` дозволило автоматизувати процес оновлення структури бази даних і дозволило уникнути ручного написання складних SQL-запитів `ALTER TABLE`. На прикладі видалення атрибута `phone_number` у моделі `teacher` було перевірено механізм відстеження змін у схемі, що підкреслило роль Prisma як інструменту підтримки актуального стану бази даних. 
У схемі `schema.prism` було впроваджено різні типи зв'язків та дотримано обмежень (constraints), згідно з наданими даними реляційної моделі. Завдяки секції
`generator client`, Prisma може згенерувати типізований клієнт за вказаним шляхом, що дозволяє виконувати складні запити та візуально перевірити роботу
обмежень таблиць. 
